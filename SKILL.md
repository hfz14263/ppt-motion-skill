...
```yaml
fills:
  - {target: WIN1, window: [60, 150, 200, 300]}    # [x, y, w, h]，pt
  - {target: WIN2, window: [380, 150, 200, 300], picture: [0, 0, 960, 540]}
```

与 `cameras:` 一样是**形状属性，不进时间轴**。`window` 必填；`picture` 缺省为整个画布
（全出血）。配上 `wipe` / `pathRight` 就是"切片依次擦出"或"取景框横扫"。

**两条硬约束，写错都不报错：**

1. **填充元素是 `<a:blipFill>`（drawingml），不是 `<p:blipFill>`。** 后者是未知元素，
   被静默忽略，形状回落到原有填充色。
2. **一个形状只能有一个填充**（`xsd:choice`，**取第一个**）。已有 `solidFill` 时
   **必须先删掉**，否则保留原色、图片永不出现。注入函数已处理。

`<a:blip r:embed>` 必须是**真实存在的关系**；编一个假 id 会画「缺图占位符」，
而它看起来和"窗口没生效"极像。图片嵌入属媒体层，不在注入函数职责内。
