...
description: 给任意 .pptx 叠加动画、切换与内嵌音视频，而完全不动版面几何。用声明式 motion spec（按 elementId 锚定）驱动 OOXML 增量注入 + Office COM 真渲染复核；动画/媒体注入后布局帧字节不变，可直接继承上游 dsh-ppt-studio 的 ppt_verify 门禁结论。附动效设计规范（节奏/时长阶梯/效果语义/页型编排）、10 套内置静态版面设计系统（另有 34 套按需抓取）、从视频反推动效节奏的分析器、方向性擦除 dir: 与 3D 相机实测表。
whenToUse: 需要给已定稿的 PPT 动态化时——加入场/强调/动作路径动画、方向性擦除、页面切换、内嵌 MP4/WAV；需要判断某份 deck 到底该不该加动效、该怎么编排节奏时；需要为静态版面挑一套命名设计系统（咨询/财务/汇报/推广/学术）时；只有一段演示视频、想量出它的动效节奏时；或需要验证"动画没有把版面搞坏"（几何自证 + 覆盖率审计 + 往返普查）；或要与 dsh-ppt-studio / open-kimi-ppt 的版面工程配合，把 elementId 变成动画目标。
---
