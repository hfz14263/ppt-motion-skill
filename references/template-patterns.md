...
**版式定义**（复现抖音教学视频得到）：同一张照片，两个版本 —— 淡化（可虚化）整图铺底
+ 原色塞进形状当前景。**只有两层**。视频里"复制 + 翻转 + 重叠"是过程演示形状，
成片前必须删掉（我复现时把它当成品层留了下来，被一眼看出"开口向右的燕尾没删"）。

两个翻车点，也正是校验器要挡住的：

| 检查 | 判据 | 标定值（实测） |
| --- | --- | --- |
| 底图与前景是否同图 | A 两张源图长宽比一致（±2%）+ B 32×32 低频结构相关 ≥ 0.80 | 同图 corr=0.950；换图 corr=0.535 |
| 前景是否被拉伸 | 失真倍数 = 目标矩形 AR ÷ 源图有效区 AR，容差 0.08 | 注入 1.15 / 1.35，量出 1.1500 / 1.3500 |

不变形的做法是 **`a:srcRect` 裁剪式放大**（先裁后拉伸），不是改 `fillRect` 或直接拉伸；
后两者正是视频里的失败对照「拉伸人物就长胖」。

    python scripts/verify_dual_photo.py FILE [--tol 0.08] [--json] [--expect-class dual-photo]
    python tests/test_dual_photo.py        # 23 条：正对照 + 拉胖/换图/空页/翻转残留 负对照

**教训两条**：① 低码率截图上的暖色中间调极易被读成独立的"设计层"（我曾把淡化背景的
中间调误判成第三块色块），指认图层前先问"这会不会只是背景照片自身的内容"；
② Windows 下 `os.path.join` 产出反斜杠，而 zip 内部路径恒为正斜杠 —— 曾因此读不到
media，校验器静默降级成 WARN 而没报错，看起来像"通过"。

样例：`examples/dual-photo-template1.pptx`（对）/ `-stretched.pptx`（拉胖 35%，会被判不通过）。
