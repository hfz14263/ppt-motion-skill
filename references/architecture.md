...
- **动画的中间态在本机不可观测**，所以方向性擦除（`wipe(left)` 到底往哪边擦）
  无法自动校准，只能靠人眼看一次。

> 脚注：开发早期曾判断"本机没有可用的对外网络"，**那个判断是错的**。
> 依据是 HTTP 请求超时，但把 HTTP 的结论套到了整个网络栈上。
> 实测 **SSH 到 github.com 是通的**（22 和 443 都握手成功，回 `Permission denied (publickey)`）。
> 教训：**一个协议失败不能推断另一个协议也不可用**，要分别测。
...
- **没有 autofit**：每个文字框必须按**真实字形**量过（PIL 读 ttc/ttf 算 advance width），
  估算是"文字压到底部色块上"的成因。
- 字体可用性要先查（本机没有思源黑体，用微软雅黑 Light 顶替）。
...
PowerShell（COM 脚本宿主，本机没有 pwsh，用 `powershell.exe`）。
...
- **覆盖率**：报告"没有任何效果"的形状。这是静默错位的主因，
  在同门课程 deck 里**先后漏了四次**（症状是"这栏从不动画"，而不是报错）。
